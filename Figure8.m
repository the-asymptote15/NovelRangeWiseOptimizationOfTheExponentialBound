...
% L_p ^(1) approximation for p=3
erfcz1 = sqrt(2/pi) * ((1 + (1./(z.^2))).^(-1/2)) .* exp(-(z.^2));
RE1 = ((abs(erfc(z)- erfcz1)) ./ erfc(z)) * 100 ; % element wise array operation

semilogy(z,RE1,'linewidth',2);
hold on
xlabel('x');
ylabel('RE');
axis([0 5 10^(-3) 10^(2)]);
...
% U_q ^(2) approximation for q=4
erfcz2 = (((sqrt(2/pi))*(2^(-1/4)))*((sqrt(2))^(-1/2))).*(z.^(-(1/2))).*(exp(-(z.^2))); 
RE2 = ((abs(erfc(z) - erfcz2)) ./ erfc(z)) * 100;
semilogy(z,RE2,'g','linewidth',2);

% U^(1) approximation
erfcz3 = sqrt(1/pi)*((4*(z.^2))+1).*(1./(2*((2*(z.^3))+z))).*(exp(-(z.^2)));
RE3 = ((abs(erfc(z) - erfcz3)) ./ erfc(z)) * 100;
semilogy(z,RE3,'linewidth',2);

% U^(3) approximation
erfcz4 = sqrt(2/pi)*((((2*(z.^2))+2).^(-0.5) + ((2)^(-(1/4))*((sqrt(2)*z).^(-0.5))))/2.01616) .*(exp(-(z.^2))); 
RE4 = ((abs(erfc(z) - erfcz4)) ./ erfc(z)) * 100;
semilogy(z,RE4,'color','#A2142F','linewidth',2);

% U^(4) approximation
erfcz5 = sqrt(1/pi)*(((4*(z.^2))+1)./(2*((2*(z.^3))+z))) .*(exp(-(z.^2))).*(1 -(exp(-(2.700907*sqrt(2)*(z)))));
RE5 = ((abs(erfc(z) - erfcz5)) ./ erfc(z)) * 100;
semilogy(z,RE5,'linewidth',2);

% U^(5) approximation
erfcz6 = sqrt(2/pi)*(((((2*(z.^2))+2).^(-0.5) +((sqrt(2)*z).^(-0.5))))/2.01616).*exp(-(z.^2)).*(1-(exp(-(4.411242*sqrt(sqrt(2)*z)))));
RE6 = ((abs(erfc(z) - erfcz6)) ./ erfc(z)) * 100;
semilogy(z,RE6,'m','linewidth',2);
...
for i = 1:points
    temp = (((abs(ERFC(i) - y(i))) )/ERFC(i)) * 100;
    RE8 = horzcat(RE8,temp);
end

semilogy(z,RE8,'r-*','linewidth',1);

%LEGEND:
h = legend('Ref. [18, L_3^{(1)}]','Ref. [18,U_4^{(2)}]','Ref. [18, U^{(1)}]','Ref. [18, U^{(3)}]','Ref. [18, U^{(4)}]','Ref. [18, U^{(5)}]','Optimized n=3','Optimized n=4');
%set(h,'location','best');
set(h,'location','southeastoutside');
set(gca,'FontSize',45) % Axis Font Setting
